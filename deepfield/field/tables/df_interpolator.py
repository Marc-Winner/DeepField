...
def interp_value_2d(x0, x1, grid, icols, ii0, ii1):
    if x0 != x0 or x1 != x1:
        return np.array([np.nan for _ in icols])
...
def interp_values_2d(xx0, xx1, grid, icols, ii0, ii1):
    """xx0, xx1 are all arrays at which values are desired
    """
...
def interp_values_3d(xx0, xx1, xx2, grid, icols, ii0, ii1, ii2):
    """xx0, xx1, xx2 are all arrays at which values are desired
    """
...
def interp_values_4d(xx0, xx1, xx2, xx3, grid, icols, ii0, ii1, ii2, ii3):
    """xx0, xx1, xx2, xx3 are all arrays at which values are desired
    """
...
def interp_values_5d(xx0, xx1, xx2, xx3, xx4, grid, icols, ii0, ii1, ii2, ii3, ii4):
    """xx0, xx1, xx2, xx3, xx4 are all arrays at which values are desired
    """
    ...
    results = np.empty((N, ncols), dtype=nb.float64)
    for i in range(N):
        res = interp_value_4d(xx0[i], xx1[i], xx2[i], xx3[i], xx4[i], grid, icols, ii0, ii1, ii2, ii3, ii4)
        for j in range(ncols):
            results[i, j] = res[j]
